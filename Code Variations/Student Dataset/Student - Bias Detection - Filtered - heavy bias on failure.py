...
data['failures_bin'] = pd.cut(data['failures'], bins=5, labels=False)
...
metrics = MetricFrame(
    ...
    y_true=y_test,
    y_pred=y_pred,
    sensitive_features=data.loc[X_test.index, 'failures_bin']
)
...
dpd = demographic_parity_difference(y_test, y_pred, sensitive_features=data.loc[X_test.index, 'failures_bin'])
eod = equalized_odds_difference(y_test, y_pred, sensitive_features=data.loc[X_test.index, 'failures_bin'])
fprd = false_positive_rate_difference(y_test, y_pred, sensitive_features=data.loc[X_test.index, 'failures_bin'])
fnrd = false_negative_rate_difference(y_test, y_pred, sensitive_features=data.loc[X_test.index, 'failures_bin'])
srd = selection_rate_difference(y_pred, sensitive_features=data.loc[X_test.index, 'failures_bin'])
for_diff = false_omission_rate_difference(y_test, y_pred, sensitive_features=data.loc[X_test.index, 'failures_bin'])
tnr_diff = true_negative_rate_difference(y_test, y_pred, sensitive_features=data.loc[X_test.index, 'failures_bin'])
...
df = pd.DataFrame({
    'y_true': y_test,
    'y_pred': y_pred,
    'failures_bin': data.loc[X_test.index, 'failures_bin']
})

# Calculate additional metrics for each subgroup
grouped_failures = df.groupby('failures_bin').apply(lambda x: pd.Series({
...
print("\nAdditional Metrics by 'failures_bin':")
print(grouped_failures)

# Plot additional metrics by 'failures_bin'
grouped_failures.plot(kind='bar', subplots=True, layout=(2, 2), figsize=(15, 10), title="Metrics by 'failures_bin'")
plt.show()
...
additional_metrics.by_group.plot(kind='bar', subplots=True, layout=(2, 2), figsize=(15, 10), title="Additional Metrics by 'failures_bin'")
plt.show()
...
relevant_features = ['failures', 'studytime', 'absences', 'age', 'G3_binary']
...
# Plot confusion matrix for failures_bin
for failure_group in df['failures_bin'].unique():
    subset = df[df['failures_bin'] == failure_group]
    ...
    plt.title(f'Confusion Matrix for Failures Group {failure_group}')
    plt.show()
