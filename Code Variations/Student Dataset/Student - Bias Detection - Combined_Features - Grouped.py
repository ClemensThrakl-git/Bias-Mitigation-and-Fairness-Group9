...
df = pd.DataFrame({
    ...
    'Dalc_bin': data.loc[X_test.index, 'Dalc_bin']
})
...
grouped_dalc = df.groupby('Dalc_bin').apply(lambda x: pd.Series({
    'accuracy': accuracy_score(x['y_true'], x['y_pred']),
    'precision': precision_score(x['y_true'], x['y_pred']),
    'recall': recall_score(x['y_true'], x['y_pred']),
    'f1': f1_score(x['y_true'], x['y_pred'])
...
print("\nAdditional Metrics by 'Dalc_bin':")
print(grouped_dalc)
...
# Plot additional metrics by 'Dalc_bin'
grouped_dalc.plot(kind='bar', subplots=True, layout=(2, 2), figsize=(15, 10), title="Metrics by 'Dalc_bin'")
plt.show()
...
# Calculate fairness metrics for 'Dalc_bin'
additional_metrics_dalc = MetricFrame(
    metrics=for_custom_metrics,
    y_true=y_test,
    y_pred=y_pred,
    sensitive_features=data.loc[X_test.index, 'Dalc_bin']
)

# Print additional metrics by group for 'Dalc_bin'
print("\nAdditional Metrics by 'Dalc_bin':")
print(additional_metrics_dalc.by_group)

# Plot additional metrics for 'Dalc_bin'
additional_metrics_dalc.by_group.plot(kind='bar', subplots=True, layout=(2, 2), figsize=(15, 10), title="Additional Metrics by 'Dalc_bin'")
plt.show()
